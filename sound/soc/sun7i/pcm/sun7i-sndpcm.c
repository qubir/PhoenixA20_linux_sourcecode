...
/*
*	i2s_pcm_select == 1:-->	pcm
*	i2s_pcm_select == 0:-->	i2s
*/

static int sun7i_pcm_set_audio_mode(struct snd_kcontrol *kcontrol,
...
static int sun7i_pcm_get_audio_mode(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new sun7i_i2s_controls[] = {
	SOC_SINGLE_BOOL_EXT("I2s Or Pcm Audio Mode Select format", 0,
			sun7i_pcm_get_audio_mode, sun7i_pcm_set_audio_mode),
};
 ...
 * Card initialization
 */
static int sun7i_pcm_init(struct snd_soc_pcm_runtime *rtd)
{
...
static struct snd_soc_ops sun7i_sndpcm_ops = {
	.hw_params 		= sun7i_sndpcm_hw_params,
};

static struct snd_soc_dai_link sun7i_sndpcm_dai_link = {
	.name 			= "PCM",
	.stream_name 	= "SUN7I-PCM",
	.cpu_dai_name 	= "sun7i-pcm.0",
	.codec_dai_name = "sndpcm",
	.init 			= sun7i_pcm_init,
	.platform_name 	= "sun7i-pcm-pcm-audio.0",
	.codec_name 	= "sun7i-pcm-codec.0",
	.ops 			= &sun7i_sndpcm_ops,
};

static struct snd_soc_card snd_soc_sun7i_sndpcm = {
	.name = "sndpcm",
	.owner 		= THIS_MODULE,
	.dai_link 	= &sun7i_sndpcm_dai_link,
	.num_links = 1,
};

static struct platform_device *sun7i_sndi2s_device;

static int __init sun7i_sndpcm_init(void)
{
	int ret; 
	...
	printk("func:%s(line:%d)sun7i-sndpcm.c,pcm_used:%d\n",__func__,__LINE__,pcm_used);
    if (pcm_used) {
		sun7i_sndi2s_device = platform_device_alloc("soc-audio", 2);
		if(!sun7i_sndi2s_device)
			return -ENOMEM;
		platform_set_drvdata(sun7i_sndi2s_device, &snd_soc_sun7i_sndpcm);
...
static void __exit sun7i_sndpcm_exit(void)
{
	if(pcm_used) {
		pcm_used = 0;
...
module_init(sun7i_sndpcm_init);
module_exit(sun7i_sndpcm_exit);
