...
static int __init sw_ahci_probe(struct platform_device *pdev)
{
	...
	if (!mem) {
        ...
        dev_err(dev, "run 000000000000000000000000000000\n");
	irq = platform_get_irq(pdev, 0);
	if (irq <= 0) {
		dev_err(dev, "no irq\n");
		return -EINVAL;
	}
	 dev_err(dev, "run 000000000000000000000000000011\n");
	if (pdata && pdata->ata_port_info)
		pi = *pdata->ata_port_info;
        dev_err(dev, "run 000000000000000000000000000022\n");
	hpriv = devm_kzalloc(dev, sizeof(*hpriv), GFP_KERNEL);
	if (!hpriv) {
		dev_err(dev, "can't alloc ahci_host_priv\n");
		return -ENOMEM;
	}
	dev_err(dev, "run 0000000000000000000000000000333\n");
...
dev_err(dev, "run 0000000000000000000000000000444\n");
	/*
	...
	if (pdata && pdata->init) {
        ...
        dev_err(dev, "run 00000000000000000000000000005555\n");
	...
	if (!host) {
        ...
        dev_err(dev, "run 00000000000000000000000000006666\n");
	...
	if (rc)
		goto err0;
dev_err(dev, "run 00000000000000000000000000007777\n");
	...
	if (rc)
		goto err0;
dev_err(dev, "run 000000000000000000000000000077777\n");
	return 0;
err0:
	if (pdata && pdata->exit)
		pdata->exit(dev);
