...
static int rtl8188eu_module_power(int onoff)
{
	...
	wifi_ldo_b = regulator_get(NULL, axp_name_b);
	if (IS_ERR(wifi_ldo_b)) {
		rtl8188eu_msg("get power regulator failed.\n");
		return -ret;
	}
	...
	if (first) {
		...
		ret = regulator_force_disable(wifi_ldo_b);
		if (ret < 0) {
			rtl8188eu_msg("regulator_force_disable fail, return %d.\n", ret);
			regulator_put(wifi_ldo_b);
			return ret;
		}
		regulator_put(wifi_ldo_b);
	...
	if (onoff) {
		...
		ret = regulator_set_voltage(wifi_ldo_b, 3300000, 3300000);
		if (ret < 0) {
			rtl8188eu_msg("regulator_set_voltage fail, return %d.\n", ret);
			regulator_put(wifi_ldo_b);
			return ret;
		...
		ret = regulator_enable(wifi_ldo_b);
		if (ret < 0) {
			rtl8188eu_msg("regulator_enable fail, return %d.\n", ret);
			regulator_put(wifi_ldo_b);
			return ret;
		...
		ret = regulator_disable(wifi_ldo_b);
		if (ret < 0) {
			rtl8188eu_msg("regulator_disable fail, return %d.\n", ret);
			regulator_put(wifi_ldo_b);
			return ret;
...
void rtl8188eu_gpio_init(void)
{
	...
	type = script_get_item(wifi_para, "rtl8188eu_shdn", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_STR != type) {
		rtl8188eu_msg("failed to fetch rtl8188eu_shdn\n");
		return ;
	}
